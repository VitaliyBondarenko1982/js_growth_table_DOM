...
function removeRow() {
    ...
    if (table.rows.length > MIN_ROWS) tbody.children[0].remove();
}
function appendColumn() {
    const rows = table.querySelectorAll("tr");
    if (rows[0].cells.length < MAX_COLUMNS) rows.forEach((row)=>{
...
function updateButtons() {
    if (table.rows.length >= MAX_ROWS) appendRowButton.setAttribute("disabled", "true");
    else appendRowButton.removeAttribute("disabled");
    if (table.rows.length <= MIN_ROWS) removeRowButton.setAttribute("disabled", "true");
    else removeRowButton.removeAttribute("disabled");
    if (table.rows[0].cells.length >= MAX_COLUMNS) appendColumnButton.setAttribute("disabled", "true");
    else appendColumnButton.removeAttribute("disabled");
    if (table.rows[0].cells.length <= MIN_COLUMNS) removeColumnButton.setAttribute("disabled", "true");
